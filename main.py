...
def get_emails_data():
    ...
    msgs = mailbox.get_messages('inbox')
    index = 1
    for msg in msgs:
...
def sort_messages_by_size(messages_data):
    return sorted(messages_data, key= lambda email_address: messages_data[email_address]['size'], reverse= True)


def sort_messages_by_quantity(messages_data):
    return sorted(messages_data, key= lambda email_address: len(messages_data[email_address]['msgs']), reverse= True)
...
if __name__ == "__main__":
    ...
    while True:
        sort_by = input('List your messages by:\n1: quantity\n2: size\nSelect (1/2)\nLeave empty to quit: ')
        if not sort_by:
            break
        if sort_by == '1':
        ...
        number_to_display = int(input('Imma list the top email addresses, How many do you want me to list?: '))
        email_addresses = email_addresses[:number_to_display]
        for index, email_address in enumerate(email_addresses):
            print(f'{index}:{email_address}: messages: {len(messages_data[email_address]["msgs"])} total size: {messages_data[email_address]["size"]}')

        while True:
            for index, email_address in enumerate(email_addresses):
                print(f'{index}:{email_address}: messages: {len(messages_data[email_address]["msgs"])} total attachments size: {str(messages_data[email_address]["size"] / (1024 * 1024))[:5]}Mb')
            selected_emails = input('Enter the numbers of email addresses you want to perform actions separated by ",", or leave empty if you are done with this selection: ')
            if not selected_emails:
                break
            action = input('What do you want to do with all the emails from those senders?\n1: Delete\n2: Move to trash\nLeave empty to select diffrent email: ')
            if not action:
                continue
